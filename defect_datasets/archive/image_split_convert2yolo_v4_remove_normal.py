...
class Xml2YOLO(object):
    ...
    def _make_train_val_dir(self):
        self._label_dir_path = os.path.join(self._xml_dir,
                                            'YOLODataset_split3/labels/')
        self._image_dir_path = os.path.join(self._xml_dir,
                                            'YOLODataset_split3/images/')
            ...
            if file_name.endswith('xml'):
                ...
                try:
                    ...
                    for item in itemlist:
                        ...
                        if tmp_label[1] in ["shuiban", "hanfeng", "yueyawan", "chongkong", "youban"]:
    ...
    def convert(self, val_size):
        ...
        for target_dir, xml_names in zip(('train/', 'val/'),
            ...
            for xml_name in xml_names:
                ...
                raw_image_path = os.path.join(self._xml_image_dir, image_name)
                try:
                    xmldoc = minidom.parse(xml_path)
                    ...
                    for img_index, (width_start, height_start, width_end, height_end) in enumerate(image_split_coord_list):
                        ...
                        for item in itemlist:
                            ...
                            if tmp_label[1] not in ["shuiban", "hanfeng", "yueyawan", "chongkong", "youban"]:
                                continue
                            ...
                            # label_id = int(tmp_label[0]) -1
                            # If the width and height of bnbox are too small, just ignore it
                        ...
                        if len(yolo_obj_list) > 0:
                            ...
                            split_img_name = xml_name.replace('.xml', '') + "_split_" + str(img_index) + ".jpg"
                            img_path = os.path.join(self._image_dir_path, target_dir, split_img_name)

                            cv2.imwrite(img_path, split_img)
                            split_xml_name = split_img_name.replace('.jpg', '.xml')
    ...
    def _save_dataset_yaml(self):
        yaml_path = os.path.join(self._xml_dir, 'YOLODataset_split3/', 'dataset.yaml')
